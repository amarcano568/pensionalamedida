$(document).on("ready", function() {
    $("#btn-carga-estrategias-hoja6").click(function(event) {
        ...
        setTimeout(function() {
            $.unblockUI();
            $("#hoja-6-fecha-nacimiento").val(
                moment($("#fechaNacimiento").val()).format("DD-MM-YYYY")
            );
            $("#hoja-6-fecha-plan").val(
                moment($("#fechaPlan").val()).format("DD-MM-YYYY")
            );
            edad = $("#divEdadCliente").text();
            $("#hoja-6-edad").val(edad);

            $("#hoja-6-fecha-baja").val($("#fechaBaja").val());

            $("#hoja-6-semanas-cotizadas").val($("#semanasCotizadas").val());
            $("#hoja-6-semanas-descontadas").val(
                $("#semanasDescontadas").val()
            );
            $("#hoja-6-total-semanas").val($("#totalSemanas").val());

            $("#hoja-6-edad-retiro").val($("#edadDe").val() + " Años");

            calcularTiempoIndividualHoja6();
            /////////////////
            edadA = $("#edadA").val();
            edadDe = $("#edadDe").val();
            ...
            $("#hoja-6-edad-desde").val(edadDe);
            $("#hoja-6-edad-hasta").val(edadA);
            $("#hoja-6-monto-pension-desde").val(
                $.number(rangoPensionDe, 2, ".", ",")
            );
            $("#hoja-6-monto-pension-hasta").val(
                $.number(rangoPensionA, 3, ".", ",")
            );
            $("#hoja-6-pagos-desde").val(
                $.number(rangoInversionDe, 2, ".", ",")
            );
            $("#hoja-6-pagos-hasta").val(
                $.number(rangoInversionA, 2, ".", ",")
            );

            $("#modal-hoja-6-estrategias").modal("show");
        }, 1500);
    });

    function calcularTiempoIndividualHoja6() {
        ...
        $.ajax({
            ...
            .done(function(response) {
                ...
                $("#hoja-6-anos-retiro").val(response.data.ano);
                $("#hoja-6-meses-retiro").val(response.data.meses);
                $("#hoja-6-semanas-retiro").val(response.data.semanas);
                $("#hoja-6-dias-retiro").val(response.data.dias);
            })
    ...
    $("#btn-carga-cotizaciones-hoja6").click(function(event) {
        event.preventDefault();
        cargaTablaCotizacionesHoja6();
    });
    function cargaTablaCotizacionesHoja6() {
        filas = $("#table-cotizaciones >tbody >tr").length + 1;
        i = 0;
        fila = $("#table-promedio-salarial-6 tr:last").attr("row");
        concepto = 6;
    ...
    function agregarTablePromedioHoja6(
        filas,
        fechaDesde,
        fechaHasta,
        dias,
        ...
        var htmlTags =
            ...
            '" id="promedio2dias' +
            filas +
            '" class="input-xs hoja-6-dias form-control" readonly value="' +
            dias +
            '"></td>' +
            '<td class="">' +
            ...
            filas +
            '" class="input-xs form-control  total-cotizacion-promedio-salarial-6" readonly value="' +
            $.number(totalMontoCotizacion, 2, ".", ",") +
            '">' +
            "</div>" +
            "</td>" +
            '<td class=""><a href="#" class="hoja-6-borrar-cotizacion"><i class="text-danger far fa-trash-alt"></i></a></td>' +
            "</tr>";

        $("#table-promedio-salarial-6 tbody").append(htmlTags);
    }

    $(".x_title_hoja6").click(function(event) {
        event.preventDefault();
        var d = new Date();
        var dia = d.getDate();
        ...
        if (NewOrEdit == "New") {
            $("#hoja-6-fecha-desde-estrategia-" + estrategia).val(fechaHoy);
            $("#hoja-6-fecha-hasta-estrategia-" + estrategia).val(fechaHoy);
            $("#hoja-6-fecha-desde-estrategia-" + estrategia).focus();
        } else {
            $("#hoja-6-fecha-desde-estrategia-" + estrategia).focus();
            // $("#hoja-2-edad-estrategia-" + estrategia).focus();
            $("#hoja-6-sbc-estrategia-" + estrategia).focus();
            $("#hoja-6-total-estrategia-" + estrategia).focus();
        }
    ...
    function calculaFechasHoja6(desde, hasta, elementoDom) {
        $.ajax({
            url: "/edad-cliente",
            type: "get",
            ...
            .done(function(response) {
                ...
                $("#hoja-6-edad-real-pension").val(response.difInDays);
            })
            .fail(function(statusCode, errorThrown) {
                $.unblockUI();
                ////console.log(errorThrown);
    ...
    $(".hoja-6-fecha-desde-estrategia").focusout(function(ev) {
        ...
        fecDesde = $("#hoja-6-fecha-desde-estrategia-" + estrategia).val();
        fecHasta = $("#hoja-6-fecha-hasta-estrategia-" + estrategia).val();
    ...
    function calculaDiasEntreFechas(fecDesde, fecHasta, estrategia) {
        $.ajax({
            ...
            .done(function(response) {
                ...
                ////console.log(response);
                $("#hoja-6-dias-estrategia-" + estrategia).val(response.data);
            })
            .fail(function(statusCode, errorThrown) {
                $.unblockUI();
                ////console.log(errorThrown);
    ...
    $(".OksumarDiasHoja6").click(function(event) {
        event.preventDefault();
        ...
        $.ajax({
            ...
            .done(function(response) {
                ...
                $("#hoja-6-sumas-dias-estrategia-" + id).hide();
                fecDesde = $("#hoja-6-fecha-desde-estrategia-" + id).val();
                fecHasta = $(fecha).val();
                calculaDiasEntreFechas(fecDesde, fecHasta, id);
                ////alert(fecDesde + " " + fecHasta);
                desde = $("#fechaNacimiento").val();
                hasta = $("#hoja-6-fecha-hasta-estrategia-" + id).val();
                ////alert(desde + " " + hasta);
                calculaFechasHoja6(
                    desde,
                    hasta,
                    "#hoja-6-edad-estrategia-" + id
                );

                setTimeout(function() {
                    dias = $("#hoja-6-dias-estrategia-" + id).val();
                    anos = parseInt(dias) / 365;
                    $("#hoja-6-anos-estrategia-" + id).val(
                        $.number(anos, 2, ".", ",")
                    );

                    meses = anos * 12;
                    $("#hoja-6-meses-estrategia-" + id).val(
                        $.number(meses, 2, ".", ",")
                    );

                    semanas = dias / 7;
                    $("#hoja-6-semanas-estrategia-" + id).val(
                        $.number(semanas, 2, ".", ",")
                    );
                    $("#hoja-6-sbc-estrategia-" + id).focus();
                }, 1000);
            })
            .fail(function(statusCode, errorThrown) {
                $.unblockUI();
    ...
    $(".sbc-monto-estrategia-hoja-6").focusout(function(ev) {
        estrategia = $(this).attr("estrategia");
        sbc = $(this).val();
        dias = $("#hoja-6-dias-estrategia-" + estrategia).val();
        total = parseFloat(sbc) * parseFloat(dias);
        $("#hoja-6-total-estrategia-" + estrategia).val(
            $.number(total, 2, ".", ",")
        );

        meses = convertNumberPure(
            $("#hoja-6-meses-estrategia-" + estrategia).val()
        );
        fecDesde = $("#hoja-6-fecha-desde-estrategia-" + estrategia).val();
        fecHasta = $("#hoja-6-fecha-hasta-estrategia-" + estrategia).val();
        switch (estrategia) {
            ...
            case "2":
                ...
                $("#hoja-6-fecha-desde-cooperativa").val(
                    moment(fecDesde).format("DD-MM-YYYY")
                );
                $("#hoja-6-fecha-hasta-cooperativa").val(
                    moment(fecHasta).format("DD-MM-YYYY")
                );
                $("#hoja-6-dias-cooperativa").val(dias);
                $("#hoja-6-sbc-cooperativa").val($.number(sbc, 2, ".", ","));
                $("#hoja-6-monto-base-cooperativa").val(
                    $.number(total, 2, ".", ",")
                );
                break;
            case "3":
                ...
                $("#hoja-6-fecha-desde-mod40-retroactivo").val(
                    moment(fecDesde).format("DD-MM-YYYY")
                );
                $("#hoja-6-fecha-hasta-mod40-retroactivo").val(
                    moment(fecHasta).format("DD-MM-YYYY")
                );
                $("#hoja-6-dias-mod40-retroactivo").val(dias);
                $("#hoja-6-sbc-mod40-retroactivo").val(
                    $.number(sbc, 2, ".", ",")
                );
                $("#hoja-6-monto-base-mod40-retroactivo").val(
                    $.number(total, 2, ".", ",")
                );
                break;
            case "4":
                ...
                $("#hoja-6-fecha-desde-m40-pagada").val(
                    moment(fecDesde).format("DD-MM-YYYY")
                );
                $("#hoja-6-fecha-hasta-m40-pagada").val(
                    moment(fecHasta).format("DD-MM-YYYY")
                );
                $("#hoja-6-dias-m40-pagada").val(dias);
                $("#hoja-6-sbc-m40-pagada").val($.number(sbc, 2, ".", ","));
                $("#hoja-6-monto-base-m40-pagada").val(
                    $.number(total, 2, ".", ",")
                );
                break;
            case "5":
                ...
                $("#hoja-6-fecha-desde-mod40-barata").val(
                    moment(fecDesde).format("DD-MM-YYYY")
                );
                $("#hoja-6-fecha-hasta-mod40-barata").val(
                    moment(fecHasta).format("DD-MM-YYYY")
                );
                $("#hoja-6-dias-mod40-barata").val(dias);
                $("#hoja-6-sbc-mod40-barata").val($.number(sbc, 2, ".", ","));
                $("#hoja-6-monto-base-mod40-barata").val(
                    $.number(total, 2, ".", ",")
                );
                break;
            case "6":
                ...
                $("#hoja-6-fecha-desde-mod40-alto").val(
                    moment(fecDesde).format("DD-MM-YYYY")
                );
                $("#hoja-6-fecha-hasta-mod40-alto").val(
                    moment(fecHasta).format("DD-MM-YYYY")
                );
                $("#hoja-6-dias-mod40-alto").val(dias);
                $("#hoja-6-sbc-mod40-alto").val($.number(sbc, 2, ".", ","));
                $("#hoja-6-monto-base-mod40-alto").val(
                    $.number(total, 2, ".", ",")
                );
                break;
        }

        changeChosenEdadPensionHoja6(estrategia);
    });

    function changeChosenEdadPensionHoja6(estrategia) {
        edadJubilacion = $("#hoja-6-edad-estrategia-" + estrategia).val();
    ...
    function calculaCostoCooperativa(estrategia) {
        ...
        inscCooperativa = convertNumberPure(
            $("#hoja-6-inscripcion-cooperativa-estrategia-" + estrategia).val()
        );
        tot1 = meses * 1750;
    ...
    $("#hoja-6-inscripcion-cooperativa-estrategia-2").focusout(function(ev) {
        estrategia = $(this).attr("estrategia");
        calculaCostoCooperativa(estrategia);
    });

    $("#modal-hoja-6-estrategias").on("show.bs.modal", function() {
        for (i = 1; i <= 6; i++) {
            dias = $("#hoja-6-dias-estrategia-" + i).val();
            if (dias != "") {
                $("#hoja-6-x_content-" + i).attr("style", "display: block;");
            } else {
                $("#hoja-6-x_content-" + i).attr("style", "display: none;");
            }
        }
    });

    // $("#modal-hoja-6-estrategias").on("hide.bs.modal", function() {
    //     totaldiasHojas6(0);
    // });
    $("#btn-cerrar-modal-hoja-6").click(function(event) {
        estrategiaEdad = $("#hoja-6-edad-calculo-pension").val();
        edadJubilacion = $("#hoja-6-edad-estrategia-" + estrategiaEdad).val();
        mes = parseInt(edadJubilacion.substr(9, 2));
        ano = parseInt(edadJubilacion.substr(0, 2));
        ...
        if (parseInt(edadPension) >= 60) {
            edadMayor = edadMayorFunc("hoja-6");
            ...
            if (edadPension < edadMayor[0].edad) {
                alertify
                    .confirm(
                        ...
                        function() {
                            totaldiasHojas6(0);
                            $("#modal-hoja-6-estrategias").modal("hide");
                        }
                    )
                    .set("labels", {
                        ok:
    ...
    function totaldiasHojas6(changeMonto) {
        diasTotal = 0;
        $(".hoja-6-dias").each(function() {
            dias = $(this).val();
            ////console.log(dias);
            if (dias != "") {
        ...
        totalCotiza = 0;
        $(".total-cotizacion-promedio-salarial-6").each(function() {
        ...
        if (diasTotal < 1750) {
            $("#hoja-6-total-dias").val($.number(diasTotal, 2, ".", ","));
            $("#hoja-6-dias-excedidos").val($.number(0, 2, ".", ","));
            $("#hoja-6-dias-equivalentes-250").val($.number(0, 2, ".", ","));
            return false;
        }
        ...
        $("#hoja-6-total-dias").val($.number(diasTotal, 2, ".", ","));
        $("#hoja-6-dias-excedidos").val($.number(diasExcedidos, 2, ".", ","));
        $("#hoja-6-dias-equivalentes-250").val(
            $.number(diasEquivalentes250, 2, ".", ",")
        );

        cargaTablaCambioSalarioHoja6();

        monto = $("#monto-a-descontar-excedido").val();

        salarioNeto = parseInt(diasExcedidos) * parseFloat(monto);

        $("#hoja-6-salarios-neto").val($.number(monto, 2, ".", ","));

        //montoExcedente = convertNumberPure($("#hoja-6-salarios-neto").val());
        salarioBasePromedio = totalCotiza - monto;
        $("#hoja-6-salario-base-promedio").val(
            $.number(salarioBasePromedio, 2, ".", ",")
        );

        promedioUltimasSemanas = salarioBasePromedio / 1750;

        $("#hoja-6-entre").val($("#hoja-6-dias-equivalentes-250").val());
        $("#hoja-6-prom-ultimas-250-sem").val(
            $.number(promedioUltimasSemanas, 2, ".", ",")
        );

        semanasCotizadas = $("#totalSemanas").val();
        semanasCotiza = 0;
        $(".semanas-cotizadas-estrategia-hoja6").each(function() {
            semanas = $(this).val();
            //console.log(semanas);
            if (semanas != "") {
        ...
        $("#hoja-6-nro-semanas-cotizadas").val(semanasCotizadas);
        $("#hoja-6-salario-promedio-mensual-250-semanas").val(
            $.number(salarioDiarioPromedio, 2, ".", ",")
        );
        $("#hoja-6-esposa").val($("#esposa").val());
        $("#hoja-6-hijos").val($("#hijos").val());
        $("#hoja-6-padres").val($("#padres").val());
        $("#hoja-6-edad-jubilacion").val(edadPension);
    ...
    $(".delete-estrategia-hoja6").click(function(event) {
        event.preventDefault();
        estrategia = $(this).attr("estrategia");
        $("#hoja-6-fecha-desde-estrategia-" + estrategia).val("");
        $("#hoja-6-fecha-hasta-estrategia-" + estrategia).val("");
        $("#hoja-6-edad-estrategia-" + estrategia).val("");
        $("#hoja-6-anos-estrategia-" + estrategia).val("");
        $("#hoja-6-meses-estrategia-" + estrategia).val("");
        $("#hoja-6-semanas-estrategia-" + estrategia).val("");
        $("#hoja-6-dias-estrategia-" + estrategia).val("");
        $("#hoja-6-sbc-estrategia-" + estrategia).val("");
        $("#hoja-6-total-estrategia-" + estrategia).val("");
        $("#hoja-6-costo-estrategia-" + estrategia).val("");
        $("#hoja-6-otro-valor-estrategia-" + estrategia).val("");
    ...
    $(document).on("click", "#cambiar-salario-calculo-hoja-6", function(event) {
        event.preventDefault();
        // if ($("#hoja-6-dias-excedidos").val() == "") {
        //     return false;
        // }
        cargaTablaCambioSalarioHoja6();
    ...
    function cargaTablaCambioSalarioHoja6() {
        i = 0;
        fila = 1;
        ...
        var filas = $("#body-promedio-salarial-6").find("tr");
        totalGeneral = 0.0;
        ...
        diasExcedidos = $("#hoja-6-dias-excedidos").val();
        ...
        //$("#hoja-6-salarios-neto").val(totalGeneral);
    ...
    function cargaFilasEstrategias() {
        dias = $("#hoja-6-dias-mod40-alto").val();
        if (dias != "") {
            concepto = "M40 -ALTO 2";
            fechaDesde = $("#hoja-6-fecha-desde-mod40-alto").val();
            fechaHasta = $("#hoja-6-fecha-hasta-mod40-alto").val();
            dias = $("#hoja-6-dias-mod40-alto").val();
            monto = $("#hoja-6-sbc-mod40-alto").val();
            agregarTableCambiosalario(
                concepto,
                fechaDesde,
                fechaHasta,
        ...
        dias = $("#hoja-6-dias-mod40-retroactivo").val();
        if (dias != "") {
            concepto = "RETROACTIVO";
            fechaDesde = $("#hoja-6-fecha-desde-mod40-retroactivo").val();
            fechaHasta = $("#hoja-6-fecha-hasta-mod40-retroactivo").val();
            dias = $("#hoja-6-dias-mod40-retroactivo").val();
            monto = $("#hoja-6-sbc-mod40-retroactivo").val();
            agregarTableCambiosalario(
                concepto,
                fechaDesde,
                fechaHasta,
        ...
        dias = $("#hoja-6-dias-mod40-barata").val();
        if (dias != "") {
            concepto = "M40 BARATA";
            fechaDesde = $("#hoja-6-fecha-desde-mod40-barata").val();
            fechaHasta = $("#hoja-6-fecha-hasta-mod40-barata").val();
            dias = $("#hoja-6-dias-mod40-barata").val();
            monto = $("#hoja-6-sbc-mod40-barata").val();
            agregarTableCambiosalario(
                concepto,
                fechaDesde,
                fechaHasta,
        ...
        dias = $("#hoja-6-dias-cooperativa").val();
        if (dias != "") {
            concepto = "COOPERATIVA";
            fechaDesde = $("#hoja-6-fecha-desde-cooperativa").val();
            fechaHasta = $("#hoja-6-fecha-hasta-cooperativa").val();
            dias = $("#hoja-6-dias-cooperativa").val();
            monto = $("#hoja-6-sbc-cooperativa").val();
            agregarTableCambiosalario(
                concepto,
                fechaDesde,
                fechaHasta,
        ...
        dias = $("#hoja-6-dias-m40-pagada").val();
        if (dias != "") {
            concepto = "M40 YA PAGADA";
            fechaDesde = $("#hoja-6-fecha-desde-m40-pagada").val();
            fechaHasta = $("#hoja-6-fecha-hasta-m40-pagada").val();
            dias = $("#hoja-6-dias-m40-pagada").val();
            monto = $("#hoja-6-sbc-m40-pagada").val();
            agregarTableCambiosalario(
                concepto,
                fechaDesde,
                fechaHasta,
    ...
    $(document).on("click", ".clickCambiaSalario", function(event) {
        ...
        // $("#hoja-6-salarios-excedidos").val(monto_sbc);
        // totaldiasHojas6(monto_sbc);
        // $("#modal-cambiar-salario").modal("hide");
    });

    $("#btn-formulas-hoja-6").click(function(event) {
        event.preventDefault();

        semanasCotizadas = $("#hoja-6-nro-semanas-cotizadas").val();
        salarioDiarioPromedio = $(
            "#hoja-6-salario-promedio-mensual-250-semanas"
        ).val();
        edadJubilacion = $("#hoja-6-edad-jubilacion").val();
        //alert(salarioDiarioPromedio);
        salarioDiarioPromedio = convertNumberPure(salarioDiarioPromedio);
        calculaFormulasExcelHojas(
            semanasCotizadas,
